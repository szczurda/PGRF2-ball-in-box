...
public class Renderer extends AbstractRenderer{
	...
	private void initTransformFeedback() {		
		...
		glBufferData(GL_ARRAY_BUFFER, 4*4*(2 + 3), GL_DYNAMIC_DRAW);//4*4*(2 + 3),
	...
	public void init() {
		...
		if (OGLUtils.getVersionGLSL() < 450)
			shaderProgramPre = ShaderUtils.loadProgram("/lvl2advanced/p07feedback/p01vs/feedbackVS");
		else
			shaderProgramPre = ShaderUtils.loadProgram("/lvl2advanced/p07feedback/p01vs/feedbackVSLayout");
		shaderProgram = ShaderUtils.loadProgram("/lvl2advanced/p07feedback/p01vs/feedbackDraw");
	...
	public void display() {
		...
		if (compute) {
			...
			FloatBuffer f = ByteBuffer.allocateDirect(4 * 4 * (2 + 3))
		...
		glDrawArrays(GL_POINTS, 0, 4); // number of elements
